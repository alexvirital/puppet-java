define java::setup (
  ...
  if ($ensure == 'present') {
    ...
    exec { "set_java_home-${name}":
      cwd     => '/',
      command => "echo 'export JAVA_HOME=/usr/java/java_latest/' >> ${pathfile}",
      unless  => "grep 'JAVA_HOME=${deploymentdir}' ${pathfile}",
      require => Exec["move_java-${name}"],
    }

     exec { "set_java_environment-${name}":
      cwd     => '/',
      command => "echo 'export JAVA_HOME=/usr/java/java_latest/' >> ${envfile}",
      unless  => "grep 'JAVA_HOME=${deploymentdir}' ${envfile}",
      require => Exec["move_java-${name}"],
    }
